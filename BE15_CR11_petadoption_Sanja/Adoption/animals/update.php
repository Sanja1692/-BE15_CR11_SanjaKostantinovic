...
if ($_GET['id']) {
    ...
    if (mysqli_num_rows($result) == 1) {
        ...
        $adoption = $data['id'];
        $resultAdop = mysqli_query($connect, "SELECT * FROM adoption");
        $adopList = "";
        if (mysqli_num_rows($resultAdop) > 0) {
            while ($row = $resultAdop->fetch_array(MYSQLI_ASSOC)) {
                if ($row['adopid'] == $adoption) {
                    $adopList .= "<option selected value='{$row['adopid']}'>{$row['adop_date']}</option>";
                } else {
                    $adopList .= "<option value='{$row['adopid']}'>{$row['adop_date']}</option>";
                }
            }
        } else {
            $adopList = "<li>There are no adoptions registered</li>";
        }
    } else {
        ...
        <form action="actions/a_update.php" method="post" enctype="multipart/form-data">
            <table class="table">
                ...
                <tr>
                    ...
                    <th>Hobby</th>
                    <td><input class="form-control" type="text" name="hobby" placeholder="Animal Hobby" value="<?php echo $hobby ?>" /></td>
                </tr><tr>
                    <th>Location</th>
